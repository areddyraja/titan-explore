 ...
 * @version $Id$ 
 */
public class TopExecutor extends AbstractExecutor
{
        ...
        if (null == query.getValues() || 0 == query.getValues().length()) {
            ...
            try {
                ...
                for (final Vertex _vertex : graph.getVertices()) {
                    maxVertices.add(GraphsUtils.wrap(_vertex, query.getKey()));
                }

                for (final Edge _edge : graph.getEdges()) {
                    maxEdges.add(GraphsUtils.wrap(_edge, query.getKey()));
                }
